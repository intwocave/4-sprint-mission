...
SELECT * FROM orders ORDER BY id DESC LIMIT 10 OFFSET 40;
...
SELECT * FROM orders WHERE date = '2025-03-12' AND time < '12:00:00.00';
...
SELECT pizza_id, COUNT(order_id) AS total_count FROM order_details GROUP BY pizza_id ORDER BY pizza_id;

-- 2. `order_details` 테이블에서 각 피자(`pizza_id`)별로 총 주문 수량을 구하세요.
SELECT pizza_id, SUM(quantity) AS total_sum FROM order_details GROUP BY pizza_id ORDER BY pizza_id;

-- 3. `pizzas` 테이블에서 `price`의 크기가 20보다 큰 피자의 종류만 `order_details` 테이블에서 조회하세요. (힌트: 서브쿼리)
SELECT * FROM order_details WHERE pizza_id IN (SELECT DISTINCT id FROM pizzas WHERE price > 20);

-- 4. `orders` 테이블에서 각 날짜별 총 주문 건수를 `order_count` 라는 이름으로 계산하고, 하루 총 주문 건수가 80건 이상인 날짜만 조회한 뒤, 주문 건수가 많은 순서대로 정렬하세요.
SELECT * FROM (SELECT date, COUNT(*) AS order_count FROM orders GROUP BY date) WHERE order_count >= 80 ORDER BY order_count DESC;

-- 5. `order_details` 테이블에서 피자별(`pizza_id`) 총 주문 수량이 10개 이상인 피자만 조회하고, 총 주문 수량 기준으로 내림차순 정렬하세요.
SELECT * FROM (SELECT pizza_id, sum(quantity) AS total_sum FROM order_details GROUP BY pizza_id) WHERE total_sum >= 10 ORDER BY total_sum DESC;

-- 6. `order_details` 테이블에서 피자별 총 수익을 `total_revenue` 라는 이름으로 구하세요. (수익 = `quantity * price`)
SELECT od.pizza_id, SUM(od.quantity * pz.price) AS total_revenue FROM order_details AS od JOIN pizzas AS pz ON od.pizza_id = pz.id GROUP BY od.pizza_id ORDER BY od.pizza_id;

-- 7. 날짜별로 피자 주문 건수(`order_count`)와 총 주문 수량(`total_quantity`)을 구하세요.
SELECT o.date, COUNT(DISTINCT o.id) AS order_count, SUM(d.quantity) AS total_quantity FROM orders AS o JOIN order_details AS d ON o.id = d.order_id GROUP BY o.date ORDER BY o.date DESC;
...
/*
    1. 피자별(`pizzas.id` 기준) 판매 수량 순위에서 피자별 판매 수량 상위에 드는 베스트 피자를 10개를 조회해 주세요. `pizzas`의 모든 컬럼을 조회하면서 각 피자에 해당하는 판매량을 `total_quantity`라는 이름으로 함께 조회합니다.
        ...
        ```
*/
SELECT p.*, SUM(d.quantity) FROM pizzas AS p JOIN order_details AS d ON p.id = d.pizza_id GROUP BY p.id ORDER BY SUM(d.quantity) DESC;
...
/*
    2. `orders` 테이블에서 2025년 3월의 일별 주문 수량을 `total_orders`라는 이름으로, 일별 총 주문 금액을 `total_amount`라는 이름으로 포함해서 조회하세요.
        ...
        ```
*/
SELECT o.date, COUNT(o.id) AS total_orders, SUM(t.sum) AS total_amount FROM orders AS o JOIN (SELECT o.order_id, SUM(o.quantity * p.price) FROM order_details AS o JOIN pizzas AS p ON o.pizza_id = p.id GROUP BY o.order_id) AS t ON o.id = t.order_id WHERE o.date BETWEEN '2025-03-01' AND '2025-03-31' GROUP BY o.date ORDER BY o.date ASC;
...
/*
    3. `order`의 `id`가 78에 해당하는 주문 내역들을 조회합니다. 주문 내역에서 각각 주문한 피자의 이름을 `pizza_name`, 피자의 크기를 `pizza_size`, 피자 가격을 `pizza_price`, 수량을 `quantity`, 각 주문 내역의 총 금액을 `total_amount` 라는 이름으로 조회해 주세요.
        ...
        ```
*/
SELECT t.name AS pizza_name, t.size AS pizza_size, t.price AS pizza_price, d.quantity, t.price * d.quantity AS total_amount FROM order_details AS d JOIN (SELECT pz.id, pz.size, pz.price, pt.name FROM pizzas AS pz JOIN pizza_types AS pt ON pz.type_id = pt.id) AS t ON d.pizza_id = t.id WHERE d.order_id = 78;
...
/*    
    4. `order_details`와 `pizzas` 테이블을 JOIN해서 피자 크기별(S, M, L) 총 수익을 계산하고, 크기별 수익을 출력하세요.
        ...
        ```
*/
SELECT pz.size, SUM(pz.price * od.quantity) AS revenue FROM pizzas AS pz JOIN order_details AS od ON pz.id = od.pizza_id GROUP BY pz.size ORDER BY SUM(pz.price * od.quantity) DESC;
...
/*    
    5. `order_details`, `pizzas`, `pizza_types` 테이블을 JOIN해서 각 피자 종류의 총 수익을 계산하고, 수익이 높은 순서대로 출력하세요.
        ...
        ```
*/
SELECT pt.name, SUM(od.quantity * pz.price) AS total_revenue FROM order_details AS od JOIN pizzas AS pz ON od.pizza_id = pz.id JOIN pizza_types AS pt ON pz.type_id = pt.id GROUP BY pt.name ORDER BY SUM(od.quantity * pz.price) DESC;
